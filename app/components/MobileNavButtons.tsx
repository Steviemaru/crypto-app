        ...
        {navItems.map((item: string) => {
          return (
            <Link key={item} href={`/${item == "Home" ? "" : item}`}>
              <button
              className={`flex flex-col items-center py-3 px-5 rounded-lg ${item == selectedItem?  "bg-black" : "bg-transparent" }`}
                onClick={() => {
                  handleClick(item);
                }}
              >
                <div className="flex justify-center md:hidden gap-5">
              {item == "Home" ? <HomeLogo className={"text-white fill-current"} width="22" height="22" /> : <CoinPageSymbol />}
            </div>
