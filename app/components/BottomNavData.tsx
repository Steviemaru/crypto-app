...
export default function BottomNavData() {
      ...
      const handleCryptoPropertyFinder = (prop:string,crypto:string) => parseFloat(data?.data[prop][crypto]);
      ...
      const totalMarketCap = handleCryptoPropertyFinder("total_market_cap", currency );
      const totalMarketVolume = handleCryptoPropertyFinder("total_volume", currency);
      const bitcoinMCP = handleCryptoPropertyFinder("market_cap_percentage","btc");
      const ethereumMCP = handleCryptoPropertyFinder("market_cap_percentage","eth");
