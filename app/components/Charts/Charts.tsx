...
import { firstLetterToUppercase, chartOptions, getChartData } from "@/utils/helperFunctions";
...
function Charts() {
  ...
  const { data: coin1Query, isLoading, isSuccess } = useGetChartDataQuery(
  ...
  const { data: coin2Query, isLoading: isLoading2, isSuccess: isSucess2 } = useGetChartDataQuery(
  ...
  const { data: coin3Query, isLoading: isLoading3, isSuccess: isSucess3 } = useGetChartDataQuery(
  ...
  const coinMarketCap = datasetArr.map((item) =>
    item.chartDataObj?.market_caps?.map((cap) => cap[1]) || []
  );

  const coinVolumes = datasetArr.map((item) =>
    item.chartDataObj?.total_volumes?.map((vol) => vol[1]) || []
  );

  // gets up to 3 market cap and total volume prices displayed onchart when carousel item is selected 
  const [coinMarketCap1, coinMarketCap2, coinMarketCap3] = coinMarketCap.map((item) => HandleFormatingNumbersAndLabels(
    parseFloat(item),
    "charts"
  ));

  const [coinTotalVolume1, coinTotalVolume2, coinTotalVolume3] = coinVolumes.map((item) => HandleFormatingNumbersAndLabels(
    parseFloat(item),
    "charts"
  ));
  ...
  const [coinName1, coinName2, coinName3] = selectedCoins.map((item) => {
    const capitalizedValue = item !== undefined ? firstLetterToUppercase(item) : item ;
  ...
  const placeholder = <div className="flex flex-col h-[500px] justify-center items-center">
    <div className="text-center text-gray-500"> Select coins to display their chart data.</div>
  </div>;

  const loader = <div className="flex flex-col h-[500px] justify-center items-center">
    <Spinner isLoading={isLoading || isLoading2 || isLoading3} />
  </div>;
  ///////////////  /////////////////////////////////////////////////////////////

  const lineChartContent = isLoading || isLoading2 || isLoading3 ? (
    loader
  ...
  const barChartContent = isLoading || isLoading2 || isLoading3 ? (
    loader
  ) : selectedCoins.length < 1 ? (
    placeholder
  ) : isSuccess ? (<BarChart
  ...
  useEffect(() => {
    const handleLoggingAndFiltering = () => {
      ...
      const queriesReady = [coin1Query, coin2Query, coin3Query].slice(0, selectedCoins.length).every((query) => query);
      ...
      if (queriesReady) {
        ...
        selectedCoins.forEach((coin, index) => {
          ...
          if (!coinExists) {
            const loggedCoin = {
              id: coin,
              chartDataObj: index === 0 ? coin1Query : index === 1 ? coin2Query : coin3Query,
            };
        ...
        setDatasetArr((prev) => prev.filter((item) => selectedCoins.includes(item.id)));
      }
    };
      ...
      <div className="flex py-4 lg:gap-10 gap-6 md:flex-row flex-col ">
        ...
        <div className="rounded-2xl p-4 bg-opacity-80 bg-shark opacity-90 md:w-3/6  ">
          <div className="lg:text-2xl text-base">
            {selectedCoins.length > 0 && <div className="mb-5 "> Market Cap</div>}
            {selectedCoins.length === 1 && (
              <div>
                <div className="flex items-center">
                  {coinName1 !== undefined && <div className="flex items-center">
            ...
            <div className="mt-6">
              <div className="flex gap-3 lg:flex-row flex-col" >
                {coinMarketCapChartLabels.map((coinData, index) => {
                  ...
                  const lineColor = colorClasses[index];
                  return name !== undefined && price !== undefined && (<div key={name} className="flex items-center" >
                    <div className={`h-4 w-4 bg-[${lineColor}] mr-3`}></div>
                    <div className="flex text-sm gap-1">
                      <div className="truncate">{name}</div><div className="">{`${symbol} ${price}`}</div></div>
                  </div>);
                })}
              </div>
            </div>)}
        ...
        <div className="rounded-2xl p-4 bg-opacity-80 bg-shark opacity-90 md:w-3/6 ">
          <div className="lg:text-2xl text-base">
            {selectedCoins.length > 0 && <div className="mb-5 "> 24hr Volume</div>}
            {selectedCoins.length === 1 && (
              <div>
                <div className="flex items-center">
                  {coinName1 !== undefined && <div className="flex items-center">
            ...
            <div className="mt-6">
              <div className="flex gap-3  lg:flex-row flex-col">
                {coinTotalVolumeChartLabels.map((coinData, index) => {
                  const [name, price] = coinData;
                  const barColor = colorClasses[index];
                  return name !== undefined && price !== undefined && (<div key={name} className="flex items-center" >
                    <div className={`h-4 w-4 bg-[${barColor}] mr-3`}></div>
                    <div className="flex text-sm gap-1">
                      <div className="truncate">{name}</div><div>{`${symbol} ${price}`}</div></div>
                  </div>);
