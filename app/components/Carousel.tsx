...
export function Carousel() {
  ...
  const { data: coinData, isLoading, isError, isSuccess } = useGetCarouselDataQuery(coinQuery);
  ...
  if (isLoading) {
    return (<div className="dark:bg-darkPurple bg-lightPurple text-lightText  flex justify-center items-center gap-10 py-4 text-sm">
            ...
            <div className="embla__viewport" ref={emblaRef}>
              <div className="embla__container">
                {coinData.length > 1 && coinData?.map((item: any) => {
                  const isSelected = selectedCoins.includes(item.id as string);
                  return (
                    <div
                      key={item.id}
                      className={` p-2  ${isSelected ? "bg-opacity-100" : "bg-opacity-70"}   bg-shark flex-none  md:w-[50%]  lg:w-[25%] flex gap-2 rounded-lg  justify-center items-center`}
                      onClick={() => {
